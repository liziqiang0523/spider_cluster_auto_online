...
class BasesiteSpider(Spider):
  ...
  start_urls = [{'url':'xxx','parser_ranks':[],'link_type':'list_url'}]
  ...
  def start_requests(self):
    ...
    for url in self.start_urls:
      ...
      req.meta['parser_rank'] = url.get('parser_ranks',[])
      req.meta['link_type'] = url.get('link_type','')
      yield req
  ...
  def parse(self, response):
    ...
    parser_ranks = response.request.meta.get('parser_ranks',[])
    print parser_ranks
    for i in parser_ranks:
      parser = self.parsers[i-1]
      ...
      if len(items) < 1:
        logging.warning('第%d个paser 没有从 url:%s中 解析到 item，不符合预期' % (i,response.reques.url))
  ...
  def extract_links(self, response, dups):
    #用target页的规则挖需要解析url
    link_type = response.request.meta.get('link_type','')
    logging.info('url:%s , link_type:[%s]' % (response.request.url,link_type))
  ...
  #使用指定的rule对response进行解析 rule={'xpath':'','allow':(),'deny':(),'parser_ranks':[]}
