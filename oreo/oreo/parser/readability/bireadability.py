...
REGEXES = {
    'unlikelyCandidatesRe': re.compile('combx|comment|community|disqus|extra|foot|header|menu|remark|rss|shoutbox|sidebar|sponsor|ad-break|agegate|pagination|pager|popup|tweet|twitter|copright', re.I),
    'okMaybeItsACandidateRe': re.compile('and|article|body|column|main|shadow|articlewrap', re.I),
    'positiveRe': re.compile('article|body|content|entry|hentry|main|page|pagination|post|text|blog|story|articlewrap', re.I),
    'negativeRe': re.compile('combx|comment|com-|contact|foot|footer|footnote|masthead|media|meta|outbrain|promo|related|scroll|shoutbox|sidebar|sponsor|shopping|tags|tool|widget|copright', re.I),
...
#regex-obj of pusblish_time of article
prog_pt = re.compile(u'((\d{4}|\d{2})(\-|\/|\.)\d{1,2}(\-|\/|\.)\d{1,2})|(\d{4}年\d{1,2}月\d{1,2}日)')
...
def clean(text):
    text = re.sub('\s*\n\s*', '\n', text)
    text = re.sub('\t|[ \t]{2,}', ' ', text)
    text = unicode(text.strip()).encode('utf-8')
    return text

reg_width = re.compile('width:(\d+)', re.I)
reg_height = re.compile('height:(\d+)', re.I)
def text_length(i, is_atag = False):
    len_text = len(clean(i.text_content() or "").decode('utf-8'))
    #print("txt:%s len_txt:%d" % (clean(i.text_content() or ""), len_text))
    if is_atag:
      return len_text
    len_img = 0
    for img in i.findall(".//img"):
        #check img_url
        img_url = img.get("src_real", '') if img.get('src_real', None) is not None else img.get('src', '')
        if img_url.split('.')[-1].upper() not in ['BMP', 'JPG', 'JPEG', 'PNG', 'GIF']:
            continue
        #length must large then 100
        width, height = to_int(img.get('width', 0)), to_int(img.get('height', 0))
        if width >= 200 and height >= 200:
            #print('shiym: good img:%s' % img.get('src', ''))
            len_img += 260
        elif width <= 0 and height <= 0:
            #find height in other attr
            for name,value in img.attrib.items():
                width, height = reg_width.search(value), reg_height.search(value)
                #if name in ['data']:
                    #print("img-src:%s value:%s re-result:%s %s" % (img.get('src'), value, width, height))
                if name in ['src', 'src_real'] or width is None or height is None: continue
                width = int(width.group(1))
                height = int(height.group(1))
                if width >= 200 and height >= 200:
                    len_img += 260
                    #print("img width:%d height:%d hidden" % (width, height))
        #big img
        if width is not None and height is not None and width * height >= 900000:
            len_img += 200
            #print("large img")
    #print("shiym: node:%s len_text:%s len_img:%d" % (describe(i), len_text, len_img))
    return len_text + len_img
    #return len_text
...
class Document:
    ...
    def __init__(self, input, positive_keywords=None, negative_keywords=None,
                 url=None, min_text_length=25, retry_length=250, xpath=False, encoding = None):
        ...
        :param retry_length:
        :param encoding:the encoding of page. default is None and get from page; if input has been encoded to certain encoding(e.g. utf8), this param should be setted.
    ...
    def _parse(self, input):
        doc, self.encoding = build_doc(input, encoding = self.encoding)
    ...
    #get post time of this page
    def post_time(self):
        '''
        title, node = shorten_title(self._html(True), is_return_title_node = True)
        if node is not None:
            #find from brother-node
            next_node = self._html(True).getnext()
            while next_node is not None:
                print("next_node:%s" % describe(next_node, depth=10))
                ret = prog_pt.search(next_node.inner_text())
                if ret is not None:
                    post_time = ret.group(1)
                    print("find post-time in [%s]" % describe(next_node, depth=10))
                    break
        '''
        post_time = ''
        node = self._html(True)
        #print("find post-time from total-content")
        for text in node.itertext():
            if len(text.strip()) < 6: continue
            #print("txt:%s" % text)
            ret = prog_pt.search(text)
            if ret is not None:
                post_time = ret.group(0)
                break
        return post_time


    def get_clean_html(self):
         #shiym edit, replace src with src_real
         for img in self.html.findall(".//img"):
             if img.get('src_real', None) is not None:
                 img.set('src', img.get('src_real'))
    ...
    def summary(self, html_partial=False):
        ...
        try:
            ...
            while True:
                ...
                #print('best_candicat:%s  len-candidates:%d' % (describe(best_candidate['elem'], depth=20), len(candidates)))
    ...
    def get_article(self, candidates, best_candidate, html_partial=False):
        ...
        #print('sibling_score_threshold:%s len-siblings:%d' % (sibling_score_threshold, len(siblings)))
        ...
        #print("----")
        #print(output)
        #print('xpath:%s text:%s  inner_text:%s' % (describe(output, depth=100), output.text, output.text_content()))
        #print("----")
        return output
    ...
    def get_link_density(self, elem):
        ...
        for i in elem.findall(".//a"):
            link_length += text_length(i, is_atag = True)
        #if len(elem.findall(".//div") or elem.findall(".//p")):
        #    link_length = link_length
        total_length = text_length(elem)
        #print('shiym: link_density of node:%s link_len:%d total_length:%d' % (describe(elem, depth=10), link_length, total_length))
    ...
    def score_paragraphs(self, ):
        ...
        for elem in self.tags(self._html(), "p", "pre", "td"):
            ...
            inner_text_len = text_length(elem)

            # If this paragraph is less than 25 characters
            # don't even count it.
            if inner_text_len < MIN_LEN:
                continue
            #print ("inner_text of %s: %s" % (elem, inner_text))
        ...
        #print('candicate-len:%d' % len(candidates))
        ...
        #print('after filter:%d' % len(candidates))
        return candidates
        ...
        for feature in [e.get('class', None), e.get('id', None)]:
            if feature:
                ...
                if self.positive_keywords and self.positive_keywords.search(feature):
                    #print ('find positive in :%s' % feature)
                    weight += 25

                if self.negative_keywords and self.negative_keywords.search(feature):
                    #print ('find nagative in :%s' % feature)
                    weight -= 25
    ...
    def sanitize(self, node, candidates):
        #print("===========article:%s" % describe(node, depth=20))
        MIN_LEN = self.min_text_length
        for header in self.tags(node, "h1", "h2", "h3", "h4", "h5", "h6"):
            if self.class_weight(header) < 0 or self.get_link_density(header) > 0.33:
                #print("header drop_tree:%s its inner_text:%s" % (header, header.text_content()))
                header.drop_tree()

        for elem in self.tags(node, "form", "textarea"):
            #print("form-textare drop_tree:%s its inner_text:%s" % (elem, elem.text_content()))
            elem.drop_tree()
        ...
        for elem in self.tags(node, "iframe"):
            if "src" in elem.attrib and REGEXES["videoRe"].search(elem.attrib["src"]):
                ...
                #print("iframe drop_tree:%s its inner_text:%s" % (elem, elem.text_content()))
                elem.drop_tree()
        ...
        for el in self.reverse_tags(node, "table", "ul", "div"):
            ...
            #print("class_weight. desc: %s with weight %s" %
                  #(describe(el), weight))
            if el in candidates:
            ...
            if weight + content_score < 0:
                ...
                #print("Removed %s with score %6.3f and weight %-3s" %
...
#                            allowed[desnode] = True
                ...
                if to_remove:
                    log.debug("Removed %6.3f %s with weight %s cause it has %s." % (content_score, describe(el, depth=10), weight, reason))
                    #print("Removed %6.3f %s with weight %s cause it has %s." % (content_score, describe(el, depth=10), weight, reason))
                    #print tounicode(el)
                    #log.debug("pname %s pweight %.3f" %(pname, pweight))
                    #print("before drop_tree[%s]:%s" % (describe(el, depth=10) , node.text_content()[:26]))
                    el.drop_tree()
                    #print("after drop_tree:%s" % node.text_content()[:26])
                else:
                    log.debug("Not removing %s of length %s: %s" % (
                        describe(el), content_length, text_content(el)))

        #print("final-node:%s text:%s" % (node, node.text_content()))
        self.html = node
