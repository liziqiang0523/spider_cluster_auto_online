...
def build_doc(page, encoding = None):
    if isinstance(page, str_):
        decoded_page = page
    else:
        if encoding is None:
            encoding = get_encoding(page) or 'utf-8'
            logging.debug("get encoding from page:%s" % encoding)
...
def add_match(collection, text, orig):
    ...
    #only english need whitespace splitting words
...
def shorten_title(doc, is_return_title_node = False):
    title = doc.find('.//title')
    if title is None or title.text is None or len(title.text) == 0:
        return ''

    candidates_node = {norm_title(title.text): title}
    ...
    for item in ['.//h1', './/h2', './/h3']:
        for e in list(doc.iterfind(item)):
            if e.text:
                text_norm = add_match(candidates, e.text, orig)
                candidates_node[text_norm] = e
            if e.text_content():
                text_norm = add_match(candidates, e.text_content(), orig)
                candidates_node[text_norm] = e
    ...
    #find node
    node = candidates_node[title] if title in candidates_node else None
    if is_return_title_node:
        return title, node
